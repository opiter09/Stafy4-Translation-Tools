...
def extract():
    ...
    binn = open("./input/arm9.bin", "rb").read()

    starts = []
    for i in range(os.stat("./input/arm9.bin").st_size):
        if (binn[i:(i + 8)].decode("UTF-8", errors = "backslashreplace") == "MESGbmg1"):
            starts.append(i)

    for i in range(len(starts)):
        size = int.from_bytes(binn[(starts[i] + 8):(starts[i] + 12)], "big")
        filePath = "./arm9_textFiles/" + str(starts[i]) + "_" + str(starts[i] + size) + ".BMG"
    ...
    for j in range(34):
        ...
        binn = open("./input/overlay_" + str(j).zfill(4) + ".bin", "rb").read()

        starts = []
        for i in range(os.stat("./input/overlay_" + str(j).zfill(4) + ".bin").st_size):
            if (binn[i:(i + 8)].decode("UTF-8", errors = "backslashreplace") == "MESGbmg1"):
                starts.append(i)
